...
func TestEndToEnd(t *testing.T) {
	...
	reportedSubtest(t, report, "identity basic temporary lifecycle", "create/list/delete/provider/environment/profile", func(t *testing.T) {
		...
		assertContains(t, listResult.stdout, "file-status=present")
		assertContains(t, listResult.stdout, "recipient=age1")
		assertContains(t, listResult.stdout, "status=ok")

		listAllIdentities := runCage(t, bin, configEnv, "", "identity", "list")
		assertContains(t, listAllIdentities.stdout, "integration-local")
		assertContains(t, listAllIdentities.stdout, "type=basic")
		assertContains(t, listAllIdentities.stdout, "file-status=present")
		assertContains(t, listAllIdentities.stdout, "recipient=age1")
		assertContains(t, listAllIdentities.stdout, "status=ok")
		...
		listProviders := runCage(t, bin, configEnv, "", "provider", "list")
		assertContains(t, listProviders.stdout, "integration-provider")
		assertContains(t, listProviders.stdout, "type=1password")
		assertContains(t, listProviders.stdout, "identity=integration-local")
		assertContains(t, listProviders.stdout, "identity-status=present")
		assertContains(t, listProviders.stdout, "file-status=age-ciphertext")
		assertContains(t, listProviders.stdout, "status=ok")
		assertNotContains(t, listProviders.stdout, fakeToken)
		...
		assertContains(t, listEnvironments.stdout, "cache=disabled")
		assertContains(t, listEnvironments.stdout, "status=ok")
		inspectEnvironment := runCage(t, bin, configEnv, "", "environment", "inspect", "integration-env")
		assertContains(t, inspectEnvironment.stdout, "Environment integration-env:")
		assertContains(t, inspectEnvironment.stdout, "provider=integration-provider")
		assertContains(t, inspectEnvironment.stdout, "provider-file-status=age-ciphertext")
		assertContains(t, inspectEnvironment.stdout, "referenced-by-profiles=-")
		assertContains(t, inspectEnvironment.stdout, "status=ok")
		...
		assertContains(t, listProfiles.stdout, "missing-environments=-")
		assertContains(t, listProfiles.stdout, "status=ok")
		inspectReferencedEnvironment := runCage(t, bin, configEnv, "", "environment", "inspect", "integration-env")
		assertContains(t, inspectReferencedEnvironment.stdout, "referenced-by-profiles=integration-profile")
